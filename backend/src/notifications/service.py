...
class NotificationService:
    ...
    async def create_verification_token(self, user_id: UUID, expires_in_hours: int = 24) -> str:
        ...
        try:
            ...
            self.supabase.table("email_verification_tokens").update({
                "expires_at": datetime.utcnow().isoformat()  # Expire immediately
            }).eq("user_id", str(user_id)).execute()

            # Create new token
            response = self.supabase.table("email_verification_tokens").insert({
                "user_id": str(user_id),
                "token": token,
    ...
    async def validate_verification_token(self, token: str) -> Optional[UUID]:
        """Validate a verification token and return user_id if valid."""
        try:
            response = self.supabase.table("email_verification_tokens").select("*").eq("token", token).single().execute()
    ...
    async def use_verification_token(self, token: str) -> bool:
